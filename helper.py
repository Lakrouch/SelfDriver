...
def get_hough_lines(img, rho=1, theta=np.pi / 180, threshold=20, min_line_len=5, max_line_gap=300):
...
def motion_planning(img, lane_line):
    rows, cols, _ = img.shape
    if len(lane_line) == 2:
        left_x2 = lane_line[0][1][0]
        right_x2 = lane_line[1][1][0]
        mid = int(cols / 2)
