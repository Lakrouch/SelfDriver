...
class LaneMemory:
    def __init__(self, max_entries=50):
        ...
        self.motion_plan = deque(maxlen=self.max_entries)
        
    def process(self, img):
        result, left_lane, right_lane, motion_plan = process_image(img, calc_mean=True, left_mem= self.left_lanes, right_mem = self.right_lanes, motion_mem=self.motion_plan)
        self.left_lanes.append(left_lane)
        self.right_lanes.append(right_lane)
        self.motion_plan.append(motion_plan)
        return result
...
def process_image(image, calc_mean=False, **kwargs):
    ...
    lane_lines_imgs.append(draw_weighted_lines(original_img, [left_lane, right_lane], thickness=15))
    ...
    if calc_mean:
        ...
        motion_plan = motion_planning(result, get_lane_lines(original_img, hough_lines))
        result = display_heading_line(result, motion_plan)
        return result, left_lane, right_lane, motion_plan
    
    result = draw_weighted_lines(original_img, [left_lane, right_lane], thickness= 10)

    motion_plan = display_heading_line(result, motion_planning(result, get_lane_lines(original_img, hough_lines)))
